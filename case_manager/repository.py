...
class CaseManager:
    ...
    def __init__(self, database_path: Path | None = None) -> None:
        self.database_path = Path(database_path) if database_path is not None else settings.case_database_path
    ...
    def create_case(self, name: str, parties: str, case_type: str) -> CaseSummary:
        ...
        with self._connect() as connection:
            cursor = connection.execute(
                "INSERT INTO cases (name, parties, case_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
                (*values, now, now),
            )
    ...
    def list_cases(self) -> list[CaseSummary]:
        with self._connect() as connection:
            rows = connection.execute("SELECT * FROM cases ORDER BY updated_at DESC, case_id DESC").fetchall()
    ...
    def list_files(self, case_id: int) -> list[StoredFile]:
        ...
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT file_id, case_id, category, filename, mime_type, length(content) AS size, created_at "
    ...
    def get_file_content(self, file_id: int) -> bytes:
        with self._connect() as connection:
            row = connection.execute("SELECT content FROM case_files WHERE file_id = ?", (file_id,)).fetchone()
        if row is None:
    ...
    def _get_record(self, record_id: int) -> CaseRecord:
        with self._connect() as connection:
            row = connection.execute("SELECT * FROM case_records WHERE record_id = ?", (record_id,)).fetchone()
        if row is None:
    ...
    def _get_file_metadata(self, file_id: int) -> StoredFile:
        with self._connect() as connection:
            row = connection.execute(
                "SELECT file_id, case_id, category, filename, mime_type, length(content) AS size, created_at "
                "FROM case_files WHERE file_id = ?", (file_id,),
            ).fetchone()
        if row is None:
            raise KeyError(f"文件不存在：{file_id}")
    ...
    def _get_event(self, event_id: int) -> CaseEvent:
        with self._connect() as connection:
            row = connection.execute("SELECT * FROM case_events WHERE event_id = ?", (event_id,)).fetchone()
        if row is None:
